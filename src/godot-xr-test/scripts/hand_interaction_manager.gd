extends Node3D
class_name HandInteractionManager
...
func _get_action_value(controller, action_path):
    if controller:
...
func _process_pinch_gesture(hand_name, pinch_value):
...
func _process_grab_gesture(hand_name, grab_value):
...
func get_hand_position(hand_name):
...
func get_hand_transform(hand_name):
    var controller = left_controller if hand_name == "left" else right_controller
