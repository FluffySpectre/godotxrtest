...
@export var max_ray_distance: float = 10.0
...
func _find_interactable_parent(node: Node) -> InteractableObject:
  var current = node
  while current && current is not InteractableObject:
    current = current.get_parent()
  
  return current if current is InteractableObject else null

func _cast_basic_ray(from: Vector3, to: Vector3) -> Dictionary:
...
func _cast_ray(from: Vector3, direction: Vector3, weight: float = 1.0) -> Dictionary:
  var to = from + (direction * max_ray_distance)
  
  var ray_results = []
  
  # Cast center ray first
  var center_result = _cast_basic_ray(from, to)
  if !center_result.is_empty():
...
func _score_candidates(candidates: Array) -> Dictionary:
  var scores = {}
  ...
  for candidate in candidates:
    ...
    if sticky_selection && selected_object == obj:
...
func _update_selection() -> void:
  if !xr_camera || selection_timer > 0:
    return
  ...
  if new_selection == null && last_frame_candidates.size() > 0:
